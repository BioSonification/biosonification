#!/usr/bin/env python3
"""Standalone script to classify consonance for example MIDI files."""

import json
import sys
from pathlib import Path

# Setup paths - use absolute paths that work everywhere
SCRIPT_DIR = Path(__file__).resolve().parent
BIOSONIFICATION_ROOT = SCRIPT_DIR
DIPLOMA_ROOT = Path("/Users/aloha_kuino/Desktop/diploma")

# Add BIOSONIFICATION to path first (for web module)
sys.path.insert(0, str(BIOSONIFICATION_ROOT))

# Add diploma to path (for crnn_music_classifier)
...
EXAMPLES_MIDI_DIR = BIOSONIFICATION_ROOT / "web" / "static" / "examples" / "midi"
EXAMPLES_DATA_FILE = BIOSONIFICATION_ROOT / "web" / "examples_data.py"
